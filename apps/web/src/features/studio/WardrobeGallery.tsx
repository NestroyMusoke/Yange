...
function GarmentThumbnail({ garment }: { garment: Garment }) {
...
export function WardrobeGallery({ garments, looks }: { garments: Garment[]; looks: LookDna[] }) {
  ...
  return (
    <section className="studio-gallery" aria-labelledby="studio-gallery-title">
      ...
      {garments.length > 0 && <div className="studio-garment-grid">{garments.map((garment) => <GarmentThumbnail key={garment.id} garment={garment} />)}</div>}
